...
describe('Icon System Integration', () => {
  const iconFiles = fs.readdirSync(iconsDir).filter(f => f.endsWith('.svelte'));
  const indexContent = fs.readFileSync(path.join(iconsDir, 'index.ts'), 'utf-8');
  ...
  function walk(dir: string) {
    ...
    for (const file of files) {
      ...
      if (fs.statSync(fullPath).isDirectory()) {
        if (file !== 'icons' && file !== 'node_modules' && file !== '.git' && file !== 'tests') {
          walk(fullPath);
        }
  ...
  const codebaseContent = allSrcFiles.map(f => fs.readFileSync(f, 'utf-8')).join('\n');
  ...
  iconFiles.forEach(file => {
    ...
    describe(`Icon: ${iconName}`, () => {
      ...
      it('should be exported in index.ts', () => {
        const exportRegex = new RegExp(`export { default as \\w+ } from "./${file}";`);
        const exportRegexSimple = new RegExp(`export { default as ${iconName} } from "./${file}";`);
        const isExported = exportRegex.test(indexContent);
        expect(isExported, `Icon ${file} is not exported in index.ts`).toBe(true);
      });
      ...
      it('should be used in the codebase', () => {
        ...
        const exportMatch = indexContent.match(new RegExp(`export { default as (\\w+) } from "./${file}";`));
        const exportedName = exportMatch ? exportMatch[1] : iconName;

        const isUsed = new RegExp(`\\b${exportedName}\\b`).test(codebaseContent) || 
                       new RegExp(`\\b${iconName}\\b`).test(codebaseContent);
        expect(isUsed, `Icon ${exportedName} (from ${file}) appears to be unused in the application`).toBe(true);
      });
    });
  });
  ...
  it('should not have any broken exports in index.ts', () => {
    ...
    if (exports) {
      exports.forEach(exp => {
        ...
        expect(exists, `Index.ts exports non-existent file: ${fileName}`).toBe(true);
      });
    }
  });
});
