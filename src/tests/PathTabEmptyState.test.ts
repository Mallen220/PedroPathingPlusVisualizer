...
vi.mock('../../actionRegistry', () => {
    ...
    return {
        actionRegistry: writable({
            ...
            wait: { kind: 'wait', label: 'Wait', isWait: true, buttonColor: 'gray', createDefault: () => ({ kind: 'wait', id: 'wait-1', durationMs: 0 }) },
            rotate: { kind: 'rotate', label: 'Rotate', isRotate: true, buttonColor: 'gray', createDefault: () => ({ kind: 'rotate', id: 'rotate-1', degrees: 0 }) },
        })
    };
});
