...
test("EmptyState appears when all items are deleted", async () => {
  ...
  // However, in PathLineSection.svelte, it is conditionally rendered: {#if lines.length > 1}
  // So currently, with 1 line, it should NOT be visible.

  const deleteButton = screen.queryByTitle("Delete Path");
  // Expect it to be present now
  expect(deleteButton).toBeInTheDocument();

  // Simulate click on the delete button (First click to confirm)
  await fireEvent.click(deleteButton!);

  // Second click to actually delete
  await fireEvent.click(deleteButton!);

  // Verify EmptyState text appears
  // Use waitFor because component update might be async
  expect(await screen.findByText("Start your path")).toBeInTheDocument();
});
