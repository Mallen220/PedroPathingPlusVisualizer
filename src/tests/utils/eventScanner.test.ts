...
describe("scanEventsInDirectory", () => {
  ...
  it("should scan directory and extract event names from valid files", async () => {
    ...
    const mockData = {
      lines: [
        {
          eventMarkers: [{ name: "Event1" }, { name: "  Event2  " }, { name: "" }]
        }
      ],
      sequence: [
    ...
    (window as any).electronAPI.readFile.mockResolvedValue(JSON.stringify(mockData));
  ...
  it("should handle parsing errors gracefully", async () => {
    ...
    (window as any).electronAPI.readFile.mockRejectedValue(new Error("Parse fail"));
