...
describe("getAlignmentMenuItems", () => {
  ...
  const startPoint = { x: 10, y: 10, locked: false, control1: {x:0, y:0}, control2: {x:0,y:0} } as any;
  const lines = [
    {
  ...
  it("should return basic menu structure", () => {
    const items = getAlignmentMenuItems(["point-0-0", "point-1-0"], startPoint, lines, onUpdate, onRecordChange);
  ...
  it("should add distribute options if more than 2 points selected", () => {
    const items = getAlignmentMenuItems(["point-0-0", "point-1-0", "point-2-0"], startPoint, lines, onUpdate, onRecordChange);
    expect(items.find(i => i.label === "Distribute Horizontally (X)")).toBeDefined();
    expect(items.find(i => i.label === "Distribute Vertically (Y)")).toBeDefined();
  });
