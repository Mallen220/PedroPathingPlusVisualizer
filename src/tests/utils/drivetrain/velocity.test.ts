...
describe("calculateDrivetrainSpeeds", () => {
  const defaultSettings = { robotImage: "none", robotDriveType: "mecanum", rWidth: 18, rLength: 18 };

  it("should return null if showRobot is false", () => {
    expect(calculateDrivetrainSpeeds(0, {}, [], {}, defaultSettings, false)).toBeNull();
  });

  it("should return null if robotImage is not 'none'", () => {
    expect(calculateDrivetrainSpeeds(0, {}, [], {}, { ...defaultSettings, robotImage: "bot.png" }, true)).toBeNull();
  });

  it("should return zeros if timeline is missing", () => {
    expect(calculateDrivetrainSpeeds(0, null, [], {}, defaultSettings, true)).toEqual({
      frontLeft: 0, backLeft: 0, frontRight: 0, backRight: 0,
    });
    expect(calculateDrivetrainSpeeds(0, { timeline: [] }, [], {}, defaultSettings, true)).toEqual({
      frontLeft: 0, backLeft: 0, frontRight: 0, backRight: 0,
    });
  });
  ...
  it("should calculate correct speeds for mecanum", () => {
    ...
    // state1
    vi.mocked(animation.calculateRobotState).mockReturnValueOnce({ x: 0, y: 0, heading: 0 });
    // state2 (dt is 0.05, so after 0.05s it moved 1 inch in X, which is forward velocity in this func. vx = 1/0.05 = 20 inches/sec)
    vi.mocked(animation.calculateRobotState).mockReturnValueOnce({ x: 1, y: 0, heading: 0 });

    const speeds = calculateDrivetrainSpeeds(0, timePrediction, [], {}, defaultSettings, true);
  ...
  it("should calculate correct angles for swerve", () => {
    ...
    const speeds = calculateDrivetrainSpeeds(0, timePrediction, [], {}, { ...defaultSettings, robotDriveType: "swerve" }, true);
  ...
  it("should handle heading wrapping correctly", () => {
    const timePrediction = { timeline: [{ endTime: 10 }] };
    vi.mocked(animation.calculateRobotState).mockReset();
    vi.mocked(animation.calculateRobotState).mockReturnValueOnce({ x: 0, y: 0, heading: 359 });
    // Moving only in X (forward for this function coordinate system)
    vi.mocked(animation.calculateRobotState).mockReturnValueOnce({ x: 0, y: 0, heading: 1 });

    const speeds = calculateDrivetrainSpeeds(0, timePrediction, [], {}, { ...defaultSettings, robotDriveType: "mecanum" }, true);
