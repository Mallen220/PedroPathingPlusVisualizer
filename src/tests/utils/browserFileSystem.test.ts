...
describe("browserFileSystem", () => {
  ...
  it("should readFile and return contents", async () => {
    const id = uniqueId();
    await browserFileSystem.writeFile(`/browser_fs/read_${id}.json`, "file data");
    const data = await browserFileSystem.readFile(`/browser_fs/read_${id}.json`);
    expect(data).toBe("file data");
  });

  it("should throw error if reading non-existent file", async () => {
    await expect(browserFileSystem.readFile(`/browser_fs/nope_${uniqueId()}.json`)).rejects.toThrow();
  });
  ...
  it("should deleteFile", async () => {
    ...
    await expect(browserFileSystem.readFile(`/browser_fs/del_${id}.json`)).rejects.toThrow();
  });

  it("should renameFile", async () => {
    const id = uniqueId();
    await browserFileSystem.writeFile(`/browser_fs/old_${id}.json`, "data");
    await browserFileSystem.renameFile(`/browser_fs/old_${id}.json`, `/browser_fs/new_${id}.json`);

    await expect(browserFileSystem.readFile(`/browser_fs/old_${id}.json`)).rejects.toThrow();
    expect(await browserFileSystem.readFile(`/browser_fs/new_${id}.json`)).toBe("data");
  });
});
