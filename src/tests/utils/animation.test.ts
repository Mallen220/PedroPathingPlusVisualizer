...
import { calculateRobotState, createAnimationController, generateOnionLayers } from "../../utils/animation";
...
describe("animation", () => {
  describe("calculateRobotState", () => {
    it("should return start point if timeline is empty", () => {
      const state = calculateRobotState(50, [], [], { x: 10, y: 20 } as any, mockScale as any, mockScale as any);
    ...
    it("should return final point if percentage is 100", () => {
      const timeline = [{ type: "travel", startTime: 0, endTime: 10, prevPoint: { x: 0, y: 0 }, lineIndex: 0, t_start: 0, t_end: 1, duration: 10, startHeading: 0, targetHeading: 90 }] as any;
      const lines = [{ endPoint: { x: 100, y: 100, heading: 'constant', degrees: 90 }, controlPoints: [] }] as any;
      const state = calculateRobotState(100, timeline, lines, { x: 0, y: 0 } as any, mockScale as any, mockScale as any);
    ...
    it("should interpolate rotation correctly on wait", () => {
      const timeline = [{ type: "wait", startTime: 0, endTime: 10, startHeading: 0, targetHeading: 90, duration: 10, atPoint: { x: 10, y: 10 } }] as any;
      // 50% through a 10s rotate event
      const state = calculateRobotState(50, timeline, [], { x: 10, y: 10 } as any, mockScale as any, mockScale as any);
      expect(state.x).toBe(10);
    ...
    it("should interpolate travel along line correctly", () => {
      const timeline = [{ type: "travel", startTime: 0, endTime: 10, prevPoint: { x: 0, y: 0 }, lineIndex: 0, duration: 10, t_start: 0, t_end: 1, startHeading: 0, targetHeading: 0 }] as any;
      const lines = [{ endPoint: { x: 100, y: 0, heading: 'constant', degrees: 0 }, controlPoints: [] }] as any;

      const state = calculateRobotState(50, timeline, lines, { x: 0, y: 0 } as any, mockScale as any, mockScale as any);
