...
import { generateLinesFromDrawing, splitPathAtPercent } from "../../utils/pathEditing";
...
describe("pathEditing", () => {
  describe("generateLinesFromDrawing", () => {
    it("should return null if less than 2 points", () => {
      expect(generateLinesFromDrawing([{ x: 0, y: 0 }], {} as any, [], [])).toBeNull();
    });
    ...
    it("should generate bezier curves from drawing points", () => {
      ...
      const startPoint = { x: 0, y: 0, locked: false, control1: {x:0,y:0}, control2: {x:0,y:0} } as Point;
  ...
  describe("splitPathAtPercent", () => {
    it("should return null if time prediction is invalid", () => {
      ...
      expect(splitPathAtPercent(50, { totalTime: 0 } as any, [], [])).toBeNull();
    });
