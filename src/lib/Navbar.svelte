...
>
      ...
      {#if $currentFilePath}
        ...
        >
          ...
          {#if settings.gitIntegration && $gitStatusStore[$currentFilePath] && $gitStatusStore[$currentFilePath] !== "clean"}
            ...
            >
              <span>{$gitStatusStore[$currentFilePath].toUpperCase()}</span>
            </div>
          {/if}
          {#if $isUnsaved}
  ...
  <div class="flex items-center gap-2 md:gap-3">
    ...
    <div class="flex items-center gap-1">
      ...
      {#if history}
        <div class="relative">
          <div bind:this={historyButtonRef}>
            <Button
    ...
    <!-- View Options -->
    <div class="relative">
      <div bind:this={viewOptionsButtonRef}>
        <Button
          variant="ghost"
          size="sm"
          className="px-2"
      ...
      {#if viewOptionsOpen}
        <div
          ...
          class="absolute right-0 mt-2 w-48 bg-white dark:bg-neutral-800 rounded-lg shadow-xl py-2 z-50 border border-neutral-200 dark:border-neutral-700 animate-in fade-in zoom-in-95 duration-100"
        >
          <div class="px-2 grid grid-cols-1 gap-1">
            <button
              on:click={() => showRuler.update((v) => !v)}
              class="flex items-center gap-2 w-full px-2 py-1.5 rounded-md hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-colors text-sm text-neutral-700 dark:text-neutral-200"
            >
              <div class={$showRuler ? "text-blue-500" : "text-neutral-400"}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
                  height="16"
            ...
            >
              <div
                class={$showProtractor ? "text-blue-500" : "text-neutral-400"}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
            ...
            <button
              ...
              class="flex items-center gap-2 w-full px-2 py-1.5 rounded-md hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-colors text-sm text-neutral-700 dark:text-neutral-200"
            >
              <div class={$showGrid ? "text-blue-500" : "text-neutral-400"}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
                  height="16"
            ...
            {#if $showGrid}
              <div class="flex items-center justify-between px-2 py-1.5 pl-8">
                <button
                  title={`Toggle Snap${getShortcutFromSettings(settings, "toggle-snap")}`}
                  on:click={() => snapToGrid.update((v) => !v)}
                  class="flex items-center gap-2 rounded-md transition-colors text-sm text-neutral-700 dark:text-neutral-200"
                >
                  <div
                    class={$snapToGrid ? "text-green-500" : "text-neutral-400"}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                ...
                <select
                  class="bg-transparent text-xs text-neutral-600 dark:text-neutral-300 focus:outline-none cursor-pointer pl-1 pr-1 py-1 hover:bg-neutral-100 dark:hover:bg-neutral-700 rounded-md transition-colors border border-neutral-200 dark:border-neutral-700"
