...
<script lang="ts">
  ...
  let { endPoint = $bindable(), locked = false, tabindex = undefined }: Props = $props();
  ...
  let isStartOutOfBounds =
    $derived(endPoint.heading === "linear" &&
    (endPoint.startDeg > 180 || endPoint.startDeg <= -180));
  let isEndOutOfBounds =
    $derived(endPoint.heading === "linear" &&
    (endPoint.endDeg > 180 || endPoint.endDeg <= -180));
  let isConstantOutOfBounds =
    $derived(endPoint.heading === "constant" &&
