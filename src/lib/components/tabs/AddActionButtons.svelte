<script lang="ts">
  ...
  export let onAddAction: (def: any) => void;
</script>
    ...
    {#if def.createDefault || def.isPath}
      <button
        on:click={() => {
          ...
          else onAddAction(def);
        }}
        title={def.isPath
          ? `Add Path${getShortcutFromSettings(settings, "add-path")}`
          : `Add ${def.label}`}
        class={`flex items-center gap-2 px-4 py-2 text-sm font-medium text-white rounded-md shadow-sm transition-colors focus:outline-none focus:ring-2 ${getButtonFilledClass(def.buttonColor || "gray")}`}
