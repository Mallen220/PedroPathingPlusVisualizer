...
>
      ...
      {:else if item.commandId}
        ...
        >
          ...
          {:else if item.iconSvg && ICON_COMPONENT_MAP[item.iconSvg]}
            ...
            />
          {:else}
            <svelte:component this={StarIcon} className="size-5 flex-none" />
          {/if}
          {#if sidebarExpanded}
            <span class="ml-3 text-sm font-medium truncate">{item.label}</span>
          {/if}
      ...
      {:else if item.type === "system"}
        {#if item.id === "fileManager"}
          ...
          <button
            ...
            class="p-1.5 rounded-md text-neutral-600 dark:text-neutral-300 hover:bg-neutral-200 dark:hover:bg-neutral-800 hover:text-purple-600 dark:hover:text-purple-400 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 flex items-center {sidebarExpanded
              ? 'w-[calc(100%-1.1rem)] px-3'
              : 'justify-center'}"
          >
            {#if item.iconComponent}
        ...
        {:else if item.id === "undo"}
          ...
          <button
            ...
            on:click={undoAction}
            disabled={!canUndo}
            class="p-1.5 rounded-md text-neutral-600 dark:text-neutral-300 hover:bg-neutral-200 dark:hover:bg-neutral-800 disabled:opacity-30 disabled:hover:bg-transparent transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 flex items-center {sidebarExpanded
              ? 'w-[calc(100%-1.1rem)] px-3'
              : 'justify-center'}"
          >
            {#if item.iconComponent}
  ...
  <div
    class="w-full flex-none p-2 border-t border-neutral-200 dark:border-neutral-800 flex flex-col items-center gap-1.5 bg-neutral-50 dark:bg-neutral-900"
  >
    <button
    ...
    >
      <div
        class="size-5 flex-none flex items-center justify-center transition-transform duration-300 {sidebarExpanded
          ? 'rotate-180'
          : ''}"
      >
        <svg
