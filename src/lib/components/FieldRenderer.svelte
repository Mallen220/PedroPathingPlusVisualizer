...
<script lang="ts">
  ...
  // Local state
  let two: Two = $state();
  let ghostRobotState: { x: number; y: number; heading: number } | null = $state(null);
  ...
  let mecanumSpeeds = $derived(calculateDrivetrainSpeeds(
    $percentStore,
    timePrediction,
    lines,
    startPoint,
  ...
  let markers =
    $derived($settingsStore?.continuousValidation || $forceShowValidation
      ? $collisionMarkers
      : []);
  ...
  let points = $derived(generatePointElements(startPoint, lines, shapes, {
    x,
    y,
    uiLength,
  ...
  let facingLineElements = $derived(generateFacingLineElements(lines, {
    x,
    y,
    robotXY,
    timePrediction,
  ...
  // Diff Event Markers
  let diffEventMarkerElements = $derived(generateDiffEventMarkerElements(
    isDiffMode,
    diffData,
    oldData,
    lines,
  ...
  // Shapes (Obstacles)
  let shapeElements = $derived(generateShapeElements(shapes, { x, y, uiLength }));
  // Onion Layers
  // Rendered as SVG overlay to avoid clearing Two.js scene.
  let onionLayerElements = $derived(generateOnionLayerElements(lines, startPoint, {
    settings,
    timePrediction,
    percentStore: $percentStore,
  }));
  // Preview Paths
  let previewPathElements = $derived(generatePreviewPathElements(
    previewOptimizedLines,
    startPoint,
    { x, y, uiLength },
  ));
  // Event Markers
  let eventMarkerElements = $derived(generateEventMarkerElements(
    lines,
    startPoint,
    sequence,
    {
  ...
  // Collision Markers
  let collisionElements = $derived(generateCollisionElements(
    markers,
    lines,
    startPoint,
    timePrediction,
