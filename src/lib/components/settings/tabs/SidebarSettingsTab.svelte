...
<script lang="ts">
  ...
  let isSidebarLayoutModified = $derived(!arraysEqual(
    settings.sidebarItems,
    DEFAULT_SIDEBAR_LAYOUT,
  ));
  let hasCustomSidebarTools = $derived((settings.customSidebarItems?.length ?? 0) > 0);
  ...
  let selectedCommand = $derived($availableCommands.find(
    (c) => c.id === customActionSelection,
  ));

  let customActionIconKey = $state(CUSTOM_ICONS.length ? CUSTOM_ICONS[0].name : "");
  let customIconSearch = $state("");
  let isIconMenuOpen = $state(false);
  let iconMenuContainer: HTMLDivElement | null = $state(null);

  let filteredCustomIcons = $derived(customIconSearch
              ...
              >
                ...
                {:else if available.iconSvg}
                  {#if ICON_COMPONENT_MAP[available.iconSvg]}
                    {@const SvelteComponent_1 = ICON_COMPONENT_MAP[available.iconSvg]}
                    <SvelteComponent_1
                      className="size-4"
                    />
                  {:else}
