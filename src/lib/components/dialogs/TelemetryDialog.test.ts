...
vi.mock("../../telemetryStore", () => ({
  importedTelemetryData: { set: vi.fn(), subscribe: vi.fn(fn => { fn(null); return () => {}; }) },
  showTelemetry: { set: vi.fn(), subscribe: vi.fn(fn => { fn(false); return () => {}; }) },
  showTelemetryGhost: { set: vi.fn(), subscribe: vi.fn(fn => { fn(false); return () => {}; }) },
  telemetryOffset: { set: vi.fn(), subscribe: vi.fn(fn => { fn(0); return () => {}; }) }
}));
