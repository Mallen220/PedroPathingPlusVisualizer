...
<script lang="ts">
  ...
  function handleDeleteExtension(id: string) {
    if (!settings.extensions) return;
    if (confirm("Are you sure you want to delete this extension?")) {
      settings.extensions = settings.extensions.filter((e) => e.id !== id);
      settings = { ...settings };
    }
  }
          ...
          >
            ...
            {#if activeTab === "robot" || searchQuery}
              <div class="section-container mb-8">
                ...
                <SettingsItem
                  label="Robot Extensions"
                  description="Define intakes, arms, or other mechanisms"
                  {searchQuery}
                  section
                >
                  <div class="flex flex-col gap-3">
                    {#if settings.extensions && settings.extensions.length > 0}
                      {#each settings.extensions as ext (ext.id)}
                        <div
                          class="p-3 bg-neutral-50 dark:bg-neutral-800/50 rounded border border-neutral-200 dark:border-neutral-700 flex flex-col gap-3"
                        >
                          <div class="flex justify-between items-center gap-2">
                            <input
                              ...
                              class="flex-1 px-2 py-1 text-sm font-medium border border-neutral-300 dark:border-neutral-600 rounded bg-white dark:bg-neutral-800 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                              placeholder="Extension Name"
                            />
                            <input
                              type="color"
                              bind:value={ext.color}
                              on:change={() => (settings = { ...settings })}
                              class="w-8 h-8 p-0.5 rounded border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 cursor-pointer"
                              title="Extension Color"
                            />
                            <button
                              on:click={() => handleDeleteExtension(ext.id)}
                          ...
                          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                            ...
                            <div class="flex flex-col gap-1">
                              <label
                                for={`ext-xoff-${ext.id}`}
                                class="text-xs text-neutral-500 dark:text-neutral-400"
                                title="Forward offset from robot center (+ is front)"
                                >Fwd Offset (in)</label
                              >
                              <input
                                id={`ext-xoff-${ext.id}`}
                                type="number"
                                bind:value={ext.xOffset}
                                on:change={() => (settings = { ...settings })}
                                step="0.5"
                                class="w-full px-2 py-1 text-sm border border-neutral-300 dark:border-neutral-600 rounded bg-white dark:bg-neutral-800 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                              />
                            ...
                            <div class="flex flex-col gap-1">
                              <label
                                for={`ext-yoff-${ext.id}`}
                                class="text-xs text-neutral-500 dark:text-neutral-400"
                                title="Right offset from robot center (+ is right)"
                                >Right Offset (in)</label
                              >
                              <input
                                id={`ext-yoff-${ext.id}`}
                                type="number"
                                bind:value={ext.yOffset}
                                on:change={() => (settings = { ...settings })}
                                step="0.5"
                                class="w-full px-2 py-1 text-sm border border-neutral-300 dark:border-neutral-600 rounded bg-white dark:bg-neutral-800 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                              />
                    ...
                    {:else}
                      <div
                        class="text-sm text-neutral-500 dark:text-neutral-400 italic text-center py-2"
                      >
                        No extensions defined.
                      </div>
                    {/if}
                    <button
                      on:click={handleAddExtension}
                      class="w-full py-2 text-sm bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-md border border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors flex items-center justify-center gap-2"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 20 20"
