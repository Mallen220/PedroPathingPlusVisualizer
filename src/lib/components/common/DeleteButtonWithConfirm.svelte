...
<script lang="ts">
  ...
  let { disabled = false, title = "Delete", className = "", ...rest }: Props = $props();
  ...
  let baseClasses = $derived(`p-1.5 rounded-md transition-all duration-200 disabled:opacity-30 flex items-center justify-center ${className}`);
  let stateClasses = $derived(confirming
