...
<script lang="ts">
  ...
  let normalizedProtractorAngle = $derived(Math.round(
  ...
  let rulerLength = $derived(Math.sqrt(
  ...
  let rulerAngle =
    $derived(Math.atan2(rulerEnd.y - rulerStart.y, rulerEnd.x - rulerStart.x) *
    (180 / Math.PI));
  ...
  let actualProtractorPos = $derived($protractorLockToRobot
