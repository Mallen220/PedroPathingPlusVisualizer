...
export async function generateSequentialCommandCode(
  ...
  packageName: string = "org.firstinspires.ftc.teamcode.Commands.AutoCommands",
  useExactPositions: boolean = false,
): Promise<string> {
  ...
  const addPose = (
    ...
    pointData?: { x: number; y: number; heading?: number },
  ): void => {
    if (!declaredPoses.has(variableName)) {
      allPoseDeclarations.push(`    private Pose ${variableName};`);
      if (useExactPositions && pointData) {
        const h =
          pointData.heading !== undefined
            ? `, Math.toRadians(${pointData.heading})`
            : ", 0";
        allPoseInitializations.push(
          `        ${variableName} = new Pose(${pointData.x.toFixed(3)}, ${pointData.y.toFixed(3)}${h});`,
        );
      } else {
        allPoseInitializations.push(
  ...
  // Resolve heading for a point
  const resolveHeading = (
    p: Point,
    isStart: boolean,
    nextLine?: Line,
  ): number => {
    if (p.heading === "constant") return p.degrees || 0;
    if (p.heading === "linear") {
      return isStart ? p.startDeg || 0 : p.endDeg || 0;
    }
    // Tangential
    if (isStart && nextLine) {
      // Calculate angle to next control point or endpoint
      const nextP =
        nextLine.controlPoints.length > 0
          ? nextLine.controlPoints[0]
          : nextLine.endPoint;
  ...
  addPose("startPoint", "startPoint", {
    x: startPoint.x,
    y: startPoint.y,
    heading: resolveHeading(startPoint, true, lines[0]),
  });
  ...
  lines.forEach((line, lineIdx) => {
    ...
    addPose(endPointName, endPointName, {
      x: line.endPoint.x,
      y: line.endPoint.y,
      heading: resolveHeading(line.endPoint, false),
    });
...
${imports}
import org.firstinspires.ftc.robotcore.external.Telemetry;
${useExactPositions ? "" : "import com.pedropathingplus.PedroPathReader;"}
...
${pathChainDeclarations}
    ...
    public ${className}(final Drivetrain drive, HardwareMap hw, Telemetry telemetry) throws IOException {
        ...
        ${useExactPositions ? "" : `PedroPathReader pp = new PedroPathReader("${fileName ? fileName.split(/[\\/]/).pop() + ".pp" || "AutoPath.pp" : "AutoPath.pp"}", hw.appContext);`}
