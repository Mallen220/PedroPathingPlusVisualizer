...
import { calculatePipelineTimePrediction } from "./engine";
...
): import("../types").TimePrediction {
  return calculatePipelineTimePrediction(startPoint, lines, settings, sequence, macros);
}
...
export function getAnimationDuration(duration: number, speed: number): number {
  if (!Number.isFinite(duration) || !Number.isFinite(speed) || speed <= 0) return 0;
