...
<script lang="ts">
  ...
  // --- Animation State ---
  let animationController: ReturnType<typeof createAnimationController> = $state();
  ...
  let hideControlTabTimeout: ReturnType<typeof setTimeout> | null = $state(null);
  ...
  let effectiveShowSidebar = $derived($isPresentationMode ? false : showSidebar);
  run(() => {
  ...
  let committedTimePrediction =
    $derived(isDiffMode && committed
      ? calculatePathTime(
  ...
  let committedTotalTime = $derived(committedTimePrediction
    ? committedTimePrediction.totalTime / 1000
    : 0);
  // If in diff mode, duration is the max of both paths
  let effectiveDuration = $derived(isDiffMode
    ? Math.max(currentTotalTime, committedTotalTime)
    : currentTotalTime);
  let animationDuration = $derived(getAnimationDuration(effectiveDuration, playbackSpeed));
  run(() => {
  ...
  let fieldRenderWidth = $derived($isPresentationMode ? mainContentWidth : fieldDrawSize);
  let fieldRenderHeight = $derived($isPresentationMode
    ? mainContentHeight
    : fieldDrawSize);
  ...
  const SvelteComponent_1 = $derived($componentRegistry.FieldRenderer || FieldRenderer);
  const SvelteComponent_2 = $derived($componentRegistry.ControlTab || ControlTab);
</script>
